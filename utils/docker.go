...
func Docker_stat(url string, num int){
	...
	check_dir()
	file, err := os.Create("stats/docker_stat_" + url[7:len(url)-6] + ".txt")
	check_error(err)
...
func Docker_cpu(url string, num int){
	...
	check_dir()
	file, err := os.Create("stats/docker_cpu_" + url[7:len(url)-6] + ".dat")
	check_error(err)
	defer file.Close()
...
func Docker_memory(url string, num int){
	...
	check_dir()
	file, err := os.Create("stats/docker_memory_" + url[7:len(url)-6] + ".dat")
	check_error(err)
	defer file.Close()
